...
async function main() {
  ...
  const email = process.env.SUPERADMIN_EMAIL || 'superadmin@INSAAN.local';
  const password = process.env.SUPERADMIN_PASSWORD || 'SuperAdmin@12345';
  const username = 'INSAAN_superadmin';
  ...
  console.log(`Password hash generated with salt rounds: 10`);
